public class Curso {
    ...
    public void cambiarHorario(String nuevoDia, String nuevoHorario) {
        // Verificar si el nuevo horario y día son válidos (implementa la lógica necesaria)
    
        // Si son válidos, actualiza los atributos
        this.diasSemana = nuevoDia.split(",");
        this.horario = nuevoHorario;
    }
    
    public Curso buscarCursoPorCodigo(String codigoCurso, Curso[] cursos) {
        for (Curso cursoActual : cursos) {
            if (cursoActual != null && cursoActual.getCodigoCurso().equals(codigoCurso)) {
                return cursoActual; // Devuelve el curso si se encuentra
            }
        }
        return null; // Devuelve null si no se encuentra el curso
    }
    // Agrega un método para obtener el código del curso
